...
import { getMp3UrlFromRapidApi } from './back-up-api';
...
export const checkPermissionAndDownload = async (videoTitle: string, videoUrl: string, videoID: string) =>
{
    if (Platform.OS === 'ios')
    {
    ...
    } else
    {
        try
        {
            ...
            if (granted === PermissionsAndroid.RESULTS.GRANTED)
            {
                ...
                const gg = await getVideoURLFromAPI(videoUrl, videoTitle)
                if (!gg)
                {
                    console.log("in")
                    const rapidApiDataUrl = await getMp3UrlFromRapidApi(videoID);
                    console.log(rapidApiDataUrl)
                    const isValid = await downloadImage(rapidApiDataUrl, videoTitle)
...
const getVideoURLFromAPI = async (videoUrl: string, title: string) =>
{
    ...
    const gg = await downloadImage(data.download_url, title)
    return gg
}
...
const downloadImage = async (videoUrl: string, title: string) =>
{
    ...
    const gg = await config(options).fetch('GET', image_URL).then(() => true).catch(() => false)
    return gg
